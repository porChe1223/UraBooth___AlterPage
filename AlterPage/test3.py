...
llm3 = ChatOpenAI(model_name="gpt-4o",
                temperature=0,
...
# ユーザーからの入力内容の種類を三つに振り分ける
def start_tool(llm, user_prompt):
    llm = llm
    # プロンプトの作成
...
def call_Analyze(llm, user_prompt, url_index=0):
    ...
    system_prompt_Evaluate = PromptTemplate(
        input_variables = ["user_prompt"],
        template =  (response.text + txt_read("resource/rag_data/ga4Info.txt")),
